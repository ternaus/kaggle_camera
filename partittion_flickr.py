...
def helper2(file_name):
    ...
    for h in range(0, h - h_step, h_step):
        for w in range(0, w - w_step, w_step):
            ...
            cv2.imwrite(
                str(new_train_path / class_name.name / (str(h) + '_' + str(w) + '_' + file_name.stem + '.jpg')), im,
                [int(cv2.IMWRITE_JPEG_QUALITY), 100])
...
for class_name in tqdm(list(flickr_path.glob('*'))):
    ...
    Parallel(n_jobs=12)(delayed(helper1)(file_name) for file_name in list(class_name.glob('*')))
...
for class_name in tqdm(list(train_path.glob('*'))):
    ...
    Parallel(n_jobs=12)(delayed(helper2)(file_name) for file_name in list(class_name.glob('*')))
